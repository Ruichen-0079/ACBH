...
<body>
<header><h1>ACBH Desktop v0.4.0-alpha6</h1><div id="busy">启动中</div></header>
<main>
<nav>
...
debug/**
*.log
*.tmp</textarea><div class="toolbar"><button onclick="saveProfile()">保存 Profile</button><button onclick="loadProfiles()">刷新 Profile</button></div><div id="profileState" class="status"></div></div><div class="panel"><h2>快照与同步</h2><label>恢复目标目录</label><input id="restoreTarget" placeholder="下载到新目录时填写"><div class="toolbar"><button onclick="profileScan()">扫描</button><button id="btnBackupUpload" onclick="profileCreate()">创建并上传</button><button id="btnBackupRestore" onclick="profileRestore()">恢复最新</button><button id="btnBackupDownload" onclick="profileDownload()">下载到新目录</button><button id="btnBackupSync" onclick="profileSync()">同步到本地</button><button onclick="post('/api/world/status')">远端状态</button></div><p id="backupDisableReason" class="muted"></p><div id="backupState" class="status"></div></div></div><div class="panel"><h2>Profiles</h2><table id="profilesTable"></table></div></section>
...
async function loadBackupSummary(){try{const j=await api('/api/backup/summary');backupServerDir.textContent=j.serverDir||'(未配置)';backupWorldDir.textContent=j.worldPath||'(未知)';backupProfileName.textContent=j.profileName||'(未创建)';const roots=j.roots||[];if(!roots.length){backupRootsTable.innerHTML='<tr><td colspan="7">暂无备份根，保存服务端目录后将自动生成</td></tr>';return}backupRootsTable.innerHTML=roots.map(r=>{const size=r.bytes!=null?String(r.bytes):'-';return '<tr><td>'+esc(r.displayName||r.rootId)+'</td><td>'+esc(r.sourcePath)+'</td><td>'+esc(r.restorePath||r.sourcePath)+'</td><td>'+(r.exists?'是':'否')+'</td><td>'+(r.pending?'是':'否')+'</td><td>'+size+'</td><td>'+(r.enabled!==false?'是':'否')+'</td></tr>'}).join('')}catch(e){backupRootsTable.innerHTML='<tr><td colspan="7">加载备份摘要失败：'+esc(e.message||e)+'</td></tr>'}}
...
function kv(o){if(o.operationId&&!o.terminalResult)return '操作已接收：'+esc(labelOp(o.name))+' / '+esc(o.operationId); if(o.terminalResult){const t=o.terminalResult;const d=t.data||{};const rows=[['outcome',t.outcome],['ok',t.ok],['errorCode',t.errorCode||d.errorCode],['message',t.message||d.message],['traceId',t.traceId||d.traceId],['field',d.field]].filter(r=>r[1]!==undefined&&r[1]!=='');return rows.map(r=>'<b>'+esc(r[0])+'</b>: '+esc(r[1])).join('<br>')+(t.warnings&&t.warnings.length?'<br><b>warnings</b>: '+t.warnings.map(esc).join('; '):'')} const rows=[]; for(const k of Object.keys(o||{}).slice(0,8)){const v=o[k]; if(typeof v!=='object')rows.push('<b>'+esc(k)+'</b>: '+esc(v))} return rows.join('<br>')||'完成'}
