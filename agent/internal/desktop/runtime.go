...
let featureGate={};
function groupCreate(){post('/api/group/create',{groupName:groupName.value,displayName:displayName.value,coordinatorUrl:groupUrl.value,requestId:'create:'+groupName.value})}
function groupJoin(){post('/api/group/join',{inviteCode:inviteCode.value,displayName:joinDisplayName.value||displayName.value,coordinatorUrl:groupUrl.value,requestId:'join:'+inviteCode.value})}
...
async function pickPath(kind,title,filter){const j=await api('/api/picker/'+kind,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title,filter})});return j.path||((j.paths||[])[0]||'')}
async function pickServerDir(){try{const p=await pickPath('folder','选择 Minecraft Server 目录');if(p)serverDir.value=p}catch(e){showError('/api/picker/folder',e)}}
async function pickLaunch(){try{const p=await pickPath('file','选择启动脚本或 JAR','*.jar;*.bat;*.cmd;*.ps1;*.sh');if(p){launchPath.value=p;if(/\.jar$/i.test(p))launchType.value='jar';else if(/\.(bat|cmd|ps1|sh)$/i.test(p))launchType.value='script'}}catch(e){showError('/api/picker/file',e)}}
async function pickJava(){try{const p=await pickPath('file','选择 Java 可执行文件','*.exe');if(p)javaPath.value=p}catch(e){showError('/api/picker/file',e)}}
...
async function saveServerConfig(){const body={serverDir:serverDir.value,launchType:launchType.value,launchPath:launchPath.value,javaPath:javaPath.value,workingDir:serverDir.value,startArgs:[],startTimeoutSeconds:120};await put('/api/config/server',body);await loadConfig()}
...
function kv(o){if(o.operationId&&!o.terminalResult)return '操作已接收：'+esc(labelOp(o.name))+' / '+esc(o.operationId); if(o.terminalResult){const t=o.terminalResult;const rows=[['outcome',t.outcome],['ok',t.ok],['errorCode',t.errorCode],['message',t.message]].filter(r=>r[1]!==undefined&&r[1]!=='');return rows.map(r=>'<b>'+esc(r[0])+'</b>: '+esc(r[1])).join('<br>')+(t.warnings&&t.warnings.length?'<br><b>warnings</b>: '+t.warnings.map(esc).join('; '):'')} const rows=[]; for(const k of Object.keys(o||{}).slice(0,8)){const v=o[k]; if(typeof v!=='object')rows.push('<b>'+esc(k)+'</b>: '+esc(v))} return rows.join('<br>')||'完成'}
