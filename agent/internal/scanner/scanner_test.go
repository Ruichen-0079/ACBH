...
func TestScanServerRuntimeIncludesRunnableDirectoryAndExcludesNoise(t *testing.T) {
	root := t.TempDir()
	for rel, content := range map[string]string{
		...
		"start.sh":                "#!/bin/sh",
		"custom/data.bin":         "custom",
		"logs/ignored.log":        "log",
		"crash-reports/crash.txt": "crash",
		"cache/item.bin":          "cache",
		".cache/item.bin":         "cache",
		"tmp/item.bin":            "tmp",
		"temp/item.bin":           "temp",
		"dist/bundle.bin":         "dist",
		"node_modules/pkg/a.js":   "node",
		".git/config":             "git",
		"session.lock":            "lock",
		"debug.tmp":               "tmp",
		".DS_Store":               "ds",
	...
	generation := 2
	opts := testOptions(root, manifest.ServerRuntime, "runtime_000001")
	opts.Generation = &generation
	got, report, err := Scan(opts)
	if err != nil {
		t.Fatalf("Scan() error = %v", err)
	}
	if got.Generation == nil || *got.Generation != generation {
		t.Fatalf("generation = %#v, want %d", got.Generation, generation)
	}
	gotPaths := make(map[string]bool)
	for _, file := range got.Files {
		gotPaths[file.Path] = true
		if file.Class != fileclass.ServerRuntime {
			t.Fatalf("file %s class = %q", file.Path, file.Class)
		}
	}
	for _, required := range []string{
		"server.properties", "world/level.dat", "mods/example.jar",
		"config/example.toml", "start.sh", "custom/data.bin",
	} {
		if !gotPaths[required] {
			t.Fatalf("required runtime file %q was excluded", required)
		}
	}
	for _, excluded := range []string{
		"logs/ignored.log", "crash-reports/crash.txt", "cache/item.bin",
		"session.lock", "debug.tmp", ".DS_Store", "Thumbs.db",
	} {
		if gotPaths[excluded] {
			t.Fatalf("excluded runtime file %q entered manifest", excluded)
		}
	}
	if report.IncludedFiles != 6 || report.IgnoredFiles < 10 || report.UnknownFiles != 0 {
		t.Fatalf("report = %#v", report)
	}
}

func TestServerRuntimeCustomExcludeRules(t *testing.T) {
	root := t.TempDir()
	writeFile(t, root, "server.properties", "motd=ACBH")
	writeFile(t, root, "private/credentials.json", "placeholder")
	generation := 0
	opts := testOptions(root, manifest.ServerRuntime, "runtime_000001")
	opts.Generation = &generation
	opts.ExcludeRules = []string{"private/"}
	got, _, err := Scan(opts)
	if err != nil {
		t.Fatalf("Scan() error = %v", err)
	}
	if len(got.Files) != 1 || got.Files[0].Path != "server.properties" {
		t.Fatalf("files = %#v", got.Files)
	}
}

func TestServerRuntimeExcludesItsManifestOutput(t *testing.T) {
	root := t.TempDir()
	writeFile(t, root, "server.properties", "motd=ACBH")
	output := filepath.Join(root, "server-runtime.manifest.json")
	writeFile(t, root, "server-runtime.manifest.json", "old manifest")
	generation := 0
	opts := testOptions(root, manifest.ServerRuntime, "runtime_000001")
	opts.Generation = &generation
