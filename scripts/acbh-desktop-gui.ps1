...
function Select-JavaFile {
    $dialog = New-Object System.Windows.Forms.OpenFileDialog
    $dialog.Title = "选择 java.exe"
    $dialog.Filter = "Java executable (java.exe)|java.exe|All files (*.*)|*.*"
    if ($dialog.ShowDialog($form) -ne [System.Windows.Forms.DialogResult]::OK) { return }
    $result = Invoke-AgentCommandSafe -ActionName "选择 Java" -Args @("desktop", "server", "select-java", "--app-data-dir", $AppDataDir, "--path", $dialog.FileName, "--json") -Json
    if ($null -ne $result -and $result.ok) {
        Add-GuiLog ("Java 已保存：" + $result.javaPath + " version=" + $result.javaVersion)
        Load-DesktopConfig
    } elseif ($null -ne $result) {
        Show-GuiError $result.message
    }
...
function Invoke-MainAction {
    ...
    if ($null -ne $result) {
        ...
        if (-not $result.ok) {
            if ($result.errorCode) { Add-GuiLog ("errorCode: " + $result.errorCode) }
            if ($result.lockPid) { Add-GuiLog ("lockPid: " + $result.lockPid) }
            if ($result.processCommandLine) { Add-GuiLog ("process: " + $result.processCommandLine) }
            if ($result.logFile) { Add-GuiLog ("logFile: " + $result.logFile) }
            if ($result.logTail) { $result.logTail | ForEach-Object { Add-GuiLog ("logTail: " + $_) } }
            if ($result.repairable) { Add-GuiLog "可点击 高级诊断 -> 修复启动状态 后重试。" }
            Show-GuiError $result.message
        }
        Refresh-Status
    }
...
function Get-SelectedSnapshot {
    if ($lstSnapshots.SelectedIndex -lt 0 -or $lstSnapshots.SelectedIndex -ge $script:SnapshotRows.Count) {
        Show-GuiError "请先选择一个远端快照。"
        return $null
    }
    return $script:SnapshotRows[$lstSnapshots.SelectedIndex]
}

function Show-SnapshotDetails {
    $snapshot = Get-SelectedSnapshot
    if ($null -eq $snapshot) { return }
    $txtSnapshotDetails.Text = @(
        "snapshotId: " + $snapshot.snapshotId,
        "status: " + $snapshot.status + "    profile: " + $snapshot.profileId,
        "createdAt: " + $snapshot.createdAt,
        "updatedAt: " + $snapshot.updatedAt + "    completedAt: " + $snapshot.completedAt,
        "files: " + $snapshot.fileCount + "    roots: " + $snapshot.rootCount,
        "logical: " + (Format-ByteSize $snapshot.logicalSize) + "    uploaded: " + (Format-ByteSize $snapshot.uploadedSize) + "    dedup: " + (Format-ByteSize $snapshot.deduplicatedSize),
        "changed: " + $snapshot.changedFileCount + "    deleted: " + $snapshot.deletedFileCount + "    warnings: " + $snapshot.warningCount,
        "traceId: " + $(if ($snapshot.traceId) { $snapshot.traceId } else { "-" })
    ) -join [Environment]::NewLine
}

function Refresh-SnapshotList {
    $result = Invoke-AgentCommandSafe -ActionName "刷新远端快照" -Args @("world-backup", "list", "--app-data-dir", $AppDataDir, "--json") -Json
    if ($null -eq $result) { return }
    $lstSnapshots.Items.Clear()
    $script:SnapshotRows = @($result.snapshots)
    foreach ($snapshot in $script:SnapshotRows) {
        $line = "{0}  {1}  {2}  {3}  files={4}  size={5}" -f $snapshot.snapshotId, $snapshot.status, $snapshot.createdAt, $snapshot.profileId, $snapshot.fileCount, (Format-ByteSize $snapshot.logicalSize)
        [void]$lstSnapshots.Items.Add($line)
    }
    if ($lstSnapshots.Items.Count -gt 0) {
        $lstSnapshots.SelectedIndex = 0
        Show-SnapshotDetails
    } else {
        $txtSnapshotDetails.Text = "暂无远端快照。"
    }
}

function Choose-RestoreTarget {
    $dialog = New-Object System.Windows.Forms.FolderBrowserDialog
    $dialog.Description = "选择下载/恢复目标目录"
    if ($txtRestoreTarget.Text) { $dialog.SelectedPath = $txtRestoreTarget.Text }
    if ($dialog.ShowDialog($form) -ne [System.Windows.Forms.DialogResult]::OK) { return }
    $txtRestoreTarget.Text = $dialog.SelectedPath
}

function Confirm-OriginalServerOverwrite {
    param([string]$TargetDir)
    $serverDir = $txtServerDir.Text.Trim()
    if (-not $serverDir -or -not $TargetDir) { return $true }
    try {
        $serverFull = [System.IO.Path]::GetFullPath($serverDir).TrimEnd('\')
        $targetFull = [System.IO.Path]::GetFullPath($TargetDir).TrimEnd('\')
        if ([string]::Equals($serverFull, $targetFull, [System.StringComparison]::OrdinalIgnoreCase)) {
            $first = [System.Windows.Forms.MessageBox]::Show("目标目录就是原服务端目录。此操作可能覆盖本地文件，是否继续？", "ACBH", "YesNo", "Warning")
            if ($first -ne [System.Windows.Forms.DialogResult]::Yes) { return $false }
            $second = [System.Windows.Forms.MessageBox]::Show("请再次确认：确定要写入原服务端目录？", "ACBH", "YesNo", "Warning")
...
function Show-SnapshotDryRun {
    param([switch]$UseRestoreTarget)
    $snapshot = Get-SelectedSnapshot
    if ($null -eq $snapshot) { return }
    $targetDir = $(if ($UseRestoreTarget) { $txtRestoreTarget.Text.Trim() } else { $txtServerDir.Text.Trim() })
    if (-not $targetDir) {
        Show-GuiError "请先选择 dry-run 目标目录。"
        return
    }
    $remote = Invoke-AgentCommandSafe -ActionName "读取快照 manifest" -Args @("world-backup", "show", $snapshot.snapshotId, "--app-data-dir", $AppDataDir, "--json") -Json
    if ($null -eq $remote) { return }
    $add = 0; $overwrite = 0; $delete = 0; $preview = @()
    foreach ($file in @($remote.manifest.files)) {
        $local = Join-Path $targetDir ($file.path -replace '/', [System.IO.Path]::DirectorySeparatorChar)
        if (-not (Test-Path -LiteralPath $local)) {
            $add++
            if ($preview.Count -lt 60) { $preview += "+ " + $file.path }
        } else {
            try {
    ...
    foreach ($deletedPath in @($remote.manifest.deletedPaths)) {
        $local = Join-Path $targetDir ($deletedPath -replace '/', [System.IO.Path]::DirectorySeparatorChar)
        if (Test-Path -LiteralPath $local) {
            $delete++
            if ($preview.Count -lt 60) { $preview += "- " + $deletedPath }
        }
    }
    $message = @(
        "Dry-run，不会写入文件。",
        "目标目录：" + $targetDir,
        "snapshotId：" + $snapshot.snapshotId,
        "新增：" + $add + "    可能覆盖：" + $overwrite + "    删除：" + $delete,
        "",
        ($preview -join [Environment]::NewLine)
    ) -join [Environment]::NewLine
    [System.Windows.Forms.MessageBox]::Show($message, "快照 dry-run", "OK", "Information") | Out-Null
}

function Download-SelectedSnapshot {
    $snapshot = Get-SelectedSnapshot
    if ($null -eq $snapshot) { return }
    $targetDir = $txtRestoreTarget.Text.Trim()
    if (-not $targetDir) {
        Show-GuiError "下载到新目录前必须先选择 targetDir。"
        return
    }
    if (-not (Confirm-OriginalServerOverwrite $targetDir)) { return }
    $result = Invoke-AgentCommandSafe -ActionName "下载快照到新目录" -Args @("world-backup", "restore", $snapshot.snapshotId, "--app-data-dir", $AppDataDir, "--server-dir", $targetDir, "--json") -Json
    if ($null -ne $result) {
        Add-GuiLog ("快照已下载/恢复到 " + $targetDir + "，snapshotId=" + $snapshot.snapshotId)
    }
}

function Restore-SelectedSnapshotToServer {
    $snapshot = Get-SelectedSnapshot
    if ($null -eq $snapshot) { return }
    $targetDir = $txtServerDir.Text.Trim()
    if (-not $targetDir) {
        Show-GuiError "请先选择 Minecraft 服务端目录。"
        return
    }
    if (-not (Confirm-OriginalServerOverwrite $targetDir)) { return }
    $result = Invoke-AgentCommandSafe -ActionName "恢复选中快照到原目录" -Args @("world-backup", "restore", $snapshot.snapshotId, "--app-data-dir", $AppDataDir, "--server-dir", $targetDir, "--json") -Json
    if ($null -ne $result) {
        Add-GuiLog ("选中快照已恢复到原目录，snapshotId=" + $snapshot.snapshotId)
    }
}

function Repair-ServerState {
    $result = Invoke-AgentCommandSafe -ActionName "修复启动状态" -Args @("desktop", "server", "repair-state", "--app-data-dir", $AppDataDir, "--json") -Json
    if ($null -ne $result) {
        Add-GuiLog $result.message
        Refresh-Status
    }
}
...
$form.Size = New-Object System.Drawing.Size(1040, 1040)
...
$setupPanel.Size = New-Object System.Drawing.Size(490, 460)
...
Add-Button $setupPanel "选择 Java" 198 418 { Select-JavaFile } ""
...
$snapshotPanel = New-Object System.Windows.Forms.GroupBox
$snapshotPanel.Text = "快照与同步"
$snapshotPanel.Location = New-Object System.Drawing.Point(20, 545)
$snapshotPanel.Size = New-Object System.Drawing.Size(980, 190)
$form.Controls.Add($snapshotPanel)

$lstSnapshots = New-Object System.Windows.Forms.ListBox
$lstSnapshots.Location = New-Object System.Drawing.Point(14, 24)
$lstSnapshots.Size = New-Object System.Drawing.Size(540, 84)
$lstSnapshots.HorizontalScrollbar = $true
$lstSnapshots.Add_SelectedIndexChanged({ Show-SnapshotDetails })
$snapshotPanel.Controls.Add($lstSnapshots)

$txtSnapshotDetails = New-Object System.Windows.Forms.TextBox
$txtSnapshotDetails.Location = New-Object System.Drawing.Point(564, 24)
$txtSnapshotDetails.Size = New-Object System.Drawing.Size(400, 84)
$txtSnapshotDetails.Multiline = $true
$txtSnapshotDetails.ScrollBars = "Vertical"
$txtSnapshotDetails.ReadOnly = $true
$snapshotPanel.Controls.Add($txtSnapshotDetails)

$txtRestoreTarget = New-Object System.Windows.Forms.TextBox
$txtRestoreTarget.Location = New-Object System.Drawing.Point(14, 116)
$txtRestoreTarget.Size = New-Object System.Drawing.Size(360, 24)
$txtRestoreTarget.ReadOnly = $true
$snapshotPanel.Controls.Add($txtRestoreTarget)

Add-Button $snapshotPanel "刷新快照列表" 384 112 { Refresh-SnapshotList } ""
Add-Button $snapshotPanel "选择目标目录" 564 112 { Choose-RestoreTarget } ""
Add-Button $snapshotPanel "下载到新目录" 744 112 { Download-SelectedSnapshot } "必须先选择 targetDir。"
Add-Button $snapshotPanel "同步 dry-run" 14 150 { Show-SnapshotDryRun } "只预览写入原服务端目录的差异。"
Add-Button $snapshotPanel "恢复 dry-run" 194 150 { Show-SnapshotDryRun -UseRestoreTarget } "只预览写入目标目录的差异。"
Add-Button $snapshotPanel "恢复选中到原目录" 374 150 { Restore-SelectedSnapshotToServer } "会二次确认覆盖原服务端目录。"

$advancedPanel = New-Object System.Windows.Forms.GroupBox
$advancedPanel.Text = "高级诊断"
$advancedPanel.Location = New-Object System.Drawing.Point(510, 745)
...
Add-Button $advancedPanel "修复启动状态" 314 92 { Repair-ServerState } ""

$btnAdvanced = Add-Button $form "高级诊断" 830 28 { $advancedPanel.Visible = -not $advancedPanel.Visible } "默认隐藏高级 CLI 能力。"

$script:LogBox = New-Object System.Windows.Forms.TextBox
$script:LogBox.Location = New-Object System.Drawing.Point(20, 885)
$script:LogBox.Size = New-Object System.Drawing.Size(980, 110)
