...
Alpha6 hotfix for alpha3 desktop user-reported issues after PR #65 merge.
...
- Windows native file pickers via `POST /api/picker/{folder,file,files}`.
- `PUT /api/config/server` validates paths, saves with read-back verification.
