...
test("server-runtime artifact is accepted and visible as latest", async (t) => {
  ...
  try {
    ...
    const latest = await app.inject({
      method: "GET",
      url: `/v1/groups/${groupId}/artifacts/latest?artifactKind=server-runtime`,
      headers: hostHeaders(hostId, hostToken),
    });
    assert.equal(latest.statusCode, 200, latest.body);
    const metadata = latest.json<{
      artifactKind: string;
      artifactId: string;
      generation: number;
      fileCount: number;
    ...
    assert.equal(metadata.artifactKind, "server-runtime");
    assert.equal(metadata.artifactId, "runtime_001");
    assert.equal(metadata.generation, 0);
    assert.equal(metadata.fileCount, 1);
    assert.equal(metadata.totalBytes, content.byteLength);
    assert.equal(metadata.creatorHostId, hostId);
  } finally {
    await app.close();
  }
});
