...
test("health and capabilities expose alpha6 protocol compatibility", async (t) => {
  ...
  try {
    ...
    const healthBody = health.json<{ version: string; coordinatorVersion: string; protocolVersion: number }>();
    assert.equal(healthBody.version, "v0.4.0-alpha6");
    assert.equal(healthBody.coordinatorVersion, "v0.4.0-alpha6");
    ...
    const body = capabilities.json<{ capabilities: string[]; coordinatorVersion: string }>();
    assert.equal(body.coordinatorVersion, "v0.4.0-alpha6");
